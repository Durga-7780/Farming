  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="font-display font-800 text-[14.5px] text-slate-950 dark:text-slate-100 tracking-tight group-hover:text-blue-600 dark:group-hover:text-sky-400 transition-colors">
          {label}
        </span>
        {Icon && (
          <div
            className={`w-9 h-9 rounded-xl border flex items-center justify-center transition-transform group-hover:scale-105 ${iconBgMap[tone]}`}
          >
            <Icon size={18} />
          </div>
        )}
      ...
      <div className="font-mono text-[24px] font-extrabold text-slate-950 dark:text-slate-100 tracking-tight tabular-nums group-hover:text-blue-600 dark:group-hover:text-sky-400 transition-colors">
        {value}
      </div>

      {sublabel && (
        <div className="text-[12px] font-semibold text-slate-600 dark:text-slate-400 mt-2 flex items-center justify-between">
          <span>{sublabel}</span>
          {isClickable && (
            <span className="text-[11.5px] text-blue-700 dark:text-sky-400 font-extrabold opacity-0 group-hover:opacity-100 transition-opacity">
              View &rarr;
            </span>
          )}
        </div>
