  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#090d16] text-slate-900 dark:text-slate-100 flex transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-[#090d16] transition-colors duration-200">
        {/* Sleek Top Header Bar - Mobile Optimized */}
        <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800 px-3 md:px-8 py-2 md:py-3 flex items-center justify-between shadow-sm transition-colors">
          {/* Left: Hamburger + Date/Time */}
          <div className="flex items-center gap-2 md:gap-3">
            <button onClick={() => setDrawerOpen(true)} className="md:hidden text-slate-700 dark:text-slate-300 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
              <Menu size={20} />
            </button>
            ...
            {/* Mobile Date/Time - Compact single line */}
            <div className="flex md:hidden items-center gap-1.5">
              <div className="w-7 h-7 rounded-lg bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 text-blue-600 dark:text-sky-400 flex items-center justify-center shrink-0">
                <Clock size={13} />
              </div>
              <div className="leading-tight">
                <div className="font-mono text-[11px] font-extrabold text-slate-950 dark:text-white">
                  {now.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </div>
                <div className="font-mono text-[10px] font-bold text-slate-500 dark:text-slate-400">
          ...
          <div className="flex items-center gap-1.5 md:gap-2.5">
            {/* Language Selector */}
            <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 md:p-1 rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-700">
              <button
                onClick={() => setLang('en')}
                className={`px-2 md:px-2.5 py-1 rounded-md md:rounded-lg text-[11px] md:text-[12px] font-extrabold transition-all ${
                  lang === 'en'
            ...
            <div className="relative" ref={notifRef}>
              <button
                ...
                className="relative p-2 md:p-2.5 rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 hover:border-blue-400 transition-colors shadow-sm"
                title="Notifications"
              >
                <Bell size={16} className="md:w-[18px] md:h-[18px]" />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 md:w-5 md:h-5 rounded-full bg-rose-600 text-white font-mono font-extrabold text-[9px] md:text-[10.5px] flex items-center justify-center border-2 border-white dark:border-slate-900 shadow-sm animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              <AnimatePresence>
                {notifOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-[calc(100vw-24px)] sm:w-[370px] max-w-[370px] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden"
                    style={{ right: '-8px' }}
                  >
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <span className="text-[13px] font-extrabold text-slate-950 dark:text-white leading-tight">{n.title}</span>
                              <span className="text-[10px] font-mono text-slate-500 shrink-0">{n.time}</span>
                            </div>
                            <p className="text-[12px] text-slate-600 dark:text-slate-400 font-medium mt-0.5 leading-snug">{n.message}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <button
              onClick={toggleTheme}
              className="p-2 md:p-2.5 rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 hover:border-blue-400 transition-colors shadow-sm"
              title={`Switch to ${isDark ? 'Light' : 'Dark'} Mode`}
            >
              {isDark ? <Sun size={16} className="text-amber-400 shrink-0 md:w-[18px] md:h-[18px]" /> : <Moon size={16} className="text-sky-400 shrink-0 md:w-[18px] md:h-[18px]" />}
            </button>

            {/* Admin Profile Pill */}
            <div className="flex items-center gap-2 md:gap-2.5 pl-1.5 md:pl-2.5 border-l border-slate-200 dark:border-slate-800">
              <div className="relative">
                <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg md:rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white font-display font-800 text-[11px] md:text-xs flex items-center justify-center shadow-md shadow-blue-500/20">
                  {user?.name?.[0]?.toUpperCase() || 'A'}
                </div>
                <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-emerald-500 border-2 border-white dark:border-slate-900" />
              </div>
              ...
              <button
                onClick={logout}
                className="hidden md:block p-1.5 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                title="Logout"
              >
                <LogOut size={16} />
              </button>
        ...
        <main className="flex-1 px-3 md:px-8 py-4 md:py-6 pb-20 md:pb-8 max-w-[1500px] w-full mx-auto overflow-x-hidden">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 px-1 py-1.5 flex items-center justify-around safe-area-bottom">
          {MOBILE_ITEMS.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex flex-col items-center gap-0.5 px-1.5 py-1.5 rounded-xl text-[9.5px] font-bold min-w-[48px] transition-all ${
                  isActive 
